...
*/
public class TextViewBoldDark extends TextView {
private Typeface tf = null;
private String customFont;
public TextViewBoldDark(Context context, AttributeSet attrs, int defStyle) {
    super(context, attrs, defStyle);
    setCustomFontTextView(context, attrs);
}

public TextViewBoldDark(Context context, AttributeSet attrs) {
    super(context, attrs);
    setCustomFontTextView(context, attrs);
}

public TextViewBoldDark(Context context) {
    super(context);

}
public boolean setCustomFontTextView(Context ctx, String asset) {
    try {
        tf = Typeface.createFromAsset(ctx.getAssets(), "museo900.ttf");
    } catch (Exception e) {
        return false;
    }
    setTypeface(tf);
